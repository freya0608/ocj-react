...
export default class BaseMoveReplay extends React.Component {
	static defaultProps = {
	...
	componentDidMount() {
		...
		if(!this.props.togoframe){
			...
			let frame={f:0}
			TweenMax.to(frame,1.5,{f:this.props.togoframe,onUpdate:()=>{
				this.dom.current.frameIndex(Math.round(frame.f))
			},onComplete:()=>{
				this.dom.current.frameIndex(Math.round(this.props.togoframe))
			}})
		}
	...
	componentDidUpdate(prevProps, prevState){
		if(this.props.togoframe&&this.props.transback){
			let frame={f:this.props.togoframe}
			TweenMax.to(frame,1.5,{f:0,onUpdate:()=>{
				if(this.dom.current){
					this.dom.current.frameIndex(Math.round(frame.f))
				}
			},onComplete:()=>{
				if(this.dom.current){
					this.dom.current.frameIndex(0)
				}
			}})
		}
	}
